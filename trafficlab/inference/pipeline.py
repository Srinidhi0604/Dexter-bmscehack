...
# Project root is two levels up from this file (trafficlab/inference/pipeline.py)
_PIPELINE_PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
...
class InferencePipeline:
    ...
    def run(self):
        ...
        pd_path = os.path.join(_PIPELINE_PROJECT_ROOT, "prior_dimensions.json")
        with open(pd_path, 'r') as f: all_priors = json.load(f)
        ...
        # Model Init — resolve weights path relative to project root if it's relative
        raw_weights = full_config['model']['weights']
        if not os.path.isabs(raw_weights):
            # Strip leading ./ or .\ if present, then join with project root
            clean = raw_weights.lstrip('./').lstrip('.\\')
            weights_path = os.path.join(_PIPELINE_PROJECT_ROOT, clean)
        else:
            weights_path = raw_weights
        self.log_fn(f"Loading Model: {weights_path}")
